...
router.post("/autores", async (req, res) => {
  ...
  const comprobacionNombre = await autorEsquema.findOne({
    nombre: autor.nombre,
  });
  const comprobacionApellido1 = await autorEsquema.findOne({
    primerApellido: autor.primerApellido,
  });
  const comprobacionApellido2 = await autorEsquema.findOne({
    sedungoApellido: autor.sedungoApellido,
  });
...
router.put("/autores/:id", (req, res) => {
  const { id } = req.params;
  const { nombre, primerApellido, sedungoApellido, libros, fecha_nac } =
    req.body;
  autorEsquema
    .updateOne(
      { _id: id },
      { $set: { nombre, primerApellido, sedungoApellido, libros, fecha_nac } }
    )
    .then((autor) => res.json(autor))
    .catch((err) => res.json(err));
});
